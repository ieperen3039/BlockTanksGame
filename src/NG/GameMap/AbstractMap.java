...
public abstract class AbstractMap implements GameMap {
    private GridRayScanner gridScanner;
     ...
     */
    protected abstract Collision getTileIntersect(Vector3fc origin, Vector3fc direction, int xCoord, int yCoord, int zCoord);
    ...
    public GridRayScanner getGridScanner() {
        if (gridScanner == null) {
            assert getMapSize() != null && getTileSize() != null;
            gridScanner = new GridRayScanner(getMapSize(), getTileSize(), this::getTileIntersect);
        }
        return gridScanner;
    }
}
